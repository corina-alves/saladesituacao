...
$sql = "SELECT * FROM boletinsmensais WHERE id = $id";
...
if (isset($_POST['atualizar'])) {
    ...
    if (!empty($arquivo)) {
        move_uploaded_file($_FILES['arquivo']['tmp_name'], '../uploadsMensais/' . $arquivo);
        $sqlUpdate = "UPDATE boletinsmensais SET data_upload='$data_upload', arquivo='$arquivo' WHERE id=$id";
    } else {
        $sqlUpdate = "UPDATE boletinsmensais SET data_upload='$data_upload' WHERE id=$id";
    }

    if ($conn->query($sqlUpdate) === TRUE) {
        header("Location: listagemBoletimMensal.php"); // Voltar pra listagem
        exit();
    } else {
...
<section class="upload">
    <div class="container">
        <div class="col-lg-6 offset-lg-3">
            <h2><strong>Editar Boletim Mensal</strong></h2>
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <p>Arquivo atual: <a href="../uploadsMensais/<?= $dados['arquivo'] ?>" target="_blank"><?= $dados['arquivo'] ?></a></p>
                    </div>
                    <button type="submit" name="atualizar" class="btn btn-success">Salvar Alterações</button>
                    <a href="listagemBoletimMensal.php" class="btn btn-secondary">Voltar</a>
                </form>
        </div>
    </div>
</section>
