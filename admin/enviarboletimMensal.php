...
<body>
  ...
  <header class="navbar">
    ...
    <section class="upload">
        <div class="container">
            <div class="col-lg-6 offset-lg-3">
                <h2>Cadastrar <strong>Boletim Mensal</strong></h2>
                <form method="post" enctype="multipart/form-data">
                    <input class="form-control" type="text" name="nome" placeholder="Nome do boletim MENSAL " required aria-label="Dar nome para o aquivo">
    ...
    if ($_POST) {
        $nome = $_POST['nome'];
        $arquivo = uniqid() . ".pdf";
        $caminho = "../uploadsMensais/" . $arquivo;

        if (move_uploaded_file($_FILES['pdf']['tmp_name'], $caminho)) {
            $stmt = $conn->prepare("INSERT INTO boletinsMensais (nome, arquivo) VALUES (?, ?)");
