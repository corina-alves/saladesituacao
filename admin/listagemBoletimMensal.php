...
$total_query = $conn->query("SELECT COUNT(*) as total FROM boletinsmensais $where");
$total_result = $total_query->fetch_assoc();
$total = $total_result['total'];
$total_paginas = ceil($total / $por_pagina);

// Consulta paginada
$sql = "SELECT * FROM boletinsmensais $where ORDER BY data_upload DESC LIMIT $inicio, $por_pagina";
...
<h1 class="mb-4">Boletins Mensais</h1>
    ...
    <tbody>
        <?php if ($result->num_rows > 0): ?>
            ...
            <tr>
                ...
                <td><a href="../uploadsMensais/<?= $row['arquivo'] ?>" target="_blank">Visualizar</a></td>
                <td>
                    <a href="editarBoletimMensal.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-warning"><i class="bi bi-pencil"></i> Editar</a>
                    <a href="excluirboletimMensal.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Deseja excluir?');"><i class="bi bi-trash"></i> Excluir</a>
                </td>
            </tr>
            <?php endwhile; ?>
        <?php else: ?>
